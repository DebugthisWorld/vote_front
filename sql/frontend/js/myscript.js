...
function computeTotal(votes, totalVotes, score,totalScore) {
	...
	if (totalVotes === 0) {
		...
		total = 3 * (votes / totalVotes * 40 + score / totalScore * 60);
	}
...
function updateResults(response) {
	...
	if (candidateNum <= 3) {
		...
		var totalScore = 0;
		for (var i = 0; i < candidateNum; i++) {
			totalVotes = totalVotes + response.data.candidates[i].votes;
			totalScore = totalScore + response.data.candidates[i].score;
		}
		// calculate total score
		for (var i = 0; i < candidateNum; i++) {
			var t = computeTotal(response.data.candidates[i].votes, totalVotes, response.data.candidates[i].score,totalScore);
			$('#c_total'+i).html("总分" + t);
		}
		$('#voting').show();
		$('#total').show();
